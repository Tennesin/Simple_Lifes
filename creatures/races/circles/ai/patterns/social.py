...
class EmpathyHelp(GoalComponent):
    ...
    def _try_help(self, ctx):
        ...
        needy = [o for o in ctx.visible_companions
                 if o.consciousness < ci_settings.SANITY_LOW_THRESHOLD
                 and c.social.get_relationship(o) >= threshold]
        if not needy:
            return None
    ...
    def _go_help(self, target_companion):
        ...
        if c.distance_to(target_companion) > ci_settings.HELP_APPROACH_DISTANCE:
            c.goal_text = ci_info.INFO_CREATURE_GOAL_HELP_APPROACH
        ...
        ) < ci_settings.CAMPFIRE_RADIUS * 0.6)

        if campfire_pos and not companion_near_fire:
            c.goal_text = ci_info.INFO_CREATURE_GOAL_HELP_LEAD
            return campfire_pos
        else:
            c.goal_text = ci_info.INFO_CREATURE_GOAL_HELP_TALK
            return (c.x, c.y)
...
class SocialResponse(GoalComponent):
    ...
    def _respond(self):
        ...
        if dist > ci_settings.TALK_DISTANCE:
            c.goal_text = ci_info.INFO_CREATURE_GOAL_SOCIAL_RESPOND_GO
            c.target = target_pos
            return target_pos
        c.goal_text = ci_info.INFO_CREATURE_GOAL_SOCIAL_RESPOND_TALK
        c.target = (c.x, c.y)
        return c.target
