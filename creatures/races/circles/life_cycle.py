...
class CreatureAging(WeakOwnerMixin):
    ...
    def update(self, dt):
        ...
        if c.age >= ci_settings.AGE_NATURAL_DEATH_START:
    ...
    def compute_stage(age):
        if age < ci_settings.AGE_CHILD_END:
            return ci_settings.LIFE_STAGE_CHILD
        elif age < ci_settings.AGE_YOUNG_ADULT_END:
            return ci_settings.LIFE_STAGE_ADULT
        else:
            return ci_settings.LIFE_STAGE_OLD
    ...
    def _apply_old_modifiers(self):
        ...
        if self._old_modifiers_applied:
            return
        c.base_speed_multiplier *= ci_settings.OLD_SPEED_MULTIPLIER
        c.memory.decay_time /= ci_settings.OLD_MEMORY_DECAY_MULTIPLIER
        c.memory.intuitive_decay_time /= ci_settings.OLD_MEMORY_DECAY_MULTIPLIER
        self._old_modifiers_applied = True

    def sync_stage_modifiers(self):
        if self.c.life_stage == ci_settings.LIFE_STAGE_OLD:
            self._apply_old_modifiers()

    def _check_natural_death(self, dt):
        c = self.c
        if c.age >= ci_settings.AGE_NATURAL_DEATH_MAX:
            c.die(ci_settings.DEATH_CAUSE_OLD_AGE)
            return
        progress = ((c.age - ci_settings.AGE_NATURAL_DEATH_START)
                    / (ci_settings.AGE_NATURAL_DEATH_MAX - ci_settings.AGE_NATURAL_DEATH_START))
        chance_per_sec = ci_settings.AGE_NATURAL_DEATH_CHANCE_PER_SEC * (1.0 + progress * 4.0)
    ...
    def effective_vision_radius(self):
        ...
        if c.life_stage == ci_settings.LIFE_STAGE_OLD:
            return ci_settings.VISION_RADIUS * ci_settings.OLD_VISION_RADIUS_MULTIPLIER
...
class CreatureFamily(WeakOwnerMixin):
    ...
    def _try_conceive(self, dt, partner, houses=None):
        c = self.c
        if c.gender != ci_settings.GENDER_FEMALE:
            return
        if c.life_stage != ci_settings.LIFE_STAGE_ADULT:
            return
        ...
        if partner is None or partner.is_dead:
            return
        if partner.life_stage != ci_settings.LIFE_STAGE_ADULT:
            return
    ...
    def _family_house_has_space(c, partner, houses):
        if not houses:
            return True
        owner_id = c.id if c.gender == ci_settings.GENDER_MALE else partner.id
    ...
    def _try_form_pair(self, other_creatures, storage_fields=None, houses=None, nearby_creatures_grid=None):
        ...
        candidate_pool = (
            nearby_creatures_grid.query_nearby(c.x, c.y, ci_settings.FAMILY_BOND_DISTANCE)
    ...
    def _resolve_pair_housing(a, b, houses):
        if not houses:
            return
        male, female = (a, b) if a.gender == ci_settings.GENDER_MALE else (b, a)
...
class CreatureTerritory(WeakOwnerMixin):
    ...
    def register_use(self, obj, resource_type, dt, campfires=None):
        c = self.c
        if c.gender != ci_settings.TERRITORY_ENABLED_GENDER or c.life_stage == ci_settings.LIFE_STAGE_OLD:
            return
        ...
        if self.usage_time[key] >= ci_settings.TERRITORY_CLAIM_TIME:
            if self.claims_count.get(resource_type, 0) < ci_settings.TERRITORY_MAX_CLAIMS_PER_TYPE:
                obj.claimed_by = c.id
    ...
    def _is_public_domain(self, obj, campfires):
        for fire in campfires:
            if math.hypot(obj.x - fire.x, obj.y - fire.y) < ci_settings.TERRITORY_PUBLIC_DOMAIN_RADIUS:
                return True
        return False
    ...
    def find_intrusion(self, visible_bushes, visible_water, visible_companions):
        ...
        for obj in my_objects:
            for other in visible_companions:
                ...
                if dist < ci_settings.TERRITORY_INTRUSION_RADIUS:
                    return (other, obj)
        return None
    ...
    def confront(self, intruder):
        c = self.c
        self.confront_cooldowns[intruder.id] = random.uniform(*ci_settings.TERRITORY_CONFRONT_COOLDOWN)
        c.social.adjust_relationship(intruder, ci_settings.TERRITORY_INTRUDER_PENALTY)
...
def _grief_death_shock_multiplier(cause, deceased_age):
    if cause == ci_settings.DEATH_CAUSE_OLD_AGE:
        return ci_settings.GRIEF_NATURAL_OLD_AGE_MULTIPLIER
    youth_ratio = 1.0 - max(0.0, min(1.0, deceased_age / ci_settings.AGE_NATURAL_DEATH_START))
    return ci_settings.GRIEF_UNNATURAL_DEATH_MULTIPLIER + youth_ratio * ci_settings.GRIEF_YOUTH_SHOCK_BONUS

def _grief_mourner_age_multiplier(life_stage):
    if life_stage == ci_settings.LIFE_STAGE_CHILD:
        return ci_settings.GRIEF_CHILD_MOURNER_MULTIPLIER
    if life_stage == ci_settings.LIFE_STAGE_OLD:
        return ci_settings.GRIEF_OLD_MOURNER_MULTIPLIER
    return 1.0
...
def _grief_kinship_penalty(deceased, mourner):
    ...
    if relationship >= ci_settings.GRIEF_ACQUAINTANCE_MIN_RELATIONSHIP:
        return ci_settings.GRIEF_BASE_PENALTY["acquaintance"]

    if ci_settings.GRIEF_WITNESS_VISION_ONLY:
        vision_radius = mourner.aging.effective_vision_radius()
        if math.hypot(mourner.x - deceased.x, mourner.y - deceased.y) < vision_radius:
            return ci_settings.GRIEF_BASE_PENALTY["witness_stranger"]
